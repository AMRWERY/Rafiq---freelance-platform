  ...
  <div>
    ...
    <div class="p-5 max-w-fit border rounded-lg space-y-5">
      ...
      <LazyVDialog
        v-model="isModalOpen"
        :size="sandboxSize"
        :role-context="isDestructive ? 'danger' : currentRole"
        tag="MILESTONE_EXECUTION"
        stage="Stage 03 / Escrow"
      ...
      >
        <div class="space-y-4 text-xs">
          <div
            class="bg-[#F7F7F9] border border-[#DEE1E7] p-3 rounded-lg flex justify-between items-center"
          >
            <div>
            ...
            <div class="text-end">
              <span class="text-[#8A909C] text-[10px] block"
                >DISBURSEMENT SUM</span
              >
              <span class="font-bold text-sm text-[#14171F]"
                >$12,500.00
          ...
          <label
            class="flex items-start gap-2 text-xs text-[#5B6270] cursor-pointer pt-1"
          >
            <input
              type="checkbox"
              v-model="agreementChecked"
              class="mt-0.5 rounded border-[#DEE1E7] text-[#2563C7]"
            />
            <span
              >I authorize instant ledger transfer from the multi-sig escrow
      ...
      <div
        class="bg-[#F7F7F9] border border-[#DEE1E7] p-3 rounded-lg flex items-center justify-between"
      >
        <LazyVRating
          v-model="sandboxRating"
          interactive
          size="md"
          :precision="2"
          show-count
          :count-label="`(${ratingCount} project ratings)`"
      ...
      <div class="bg-white border border-[#DEE1E7] p-5 rounded-xl space-y-4">
        <LazyVSelectInput
          v-model="sandboxSelectValue"
          label="Preferred Contract Model"
          placeholder="Select a contract model"
          :options="[
        ...
        <LazyVTextarea
          v-model="sandboxTextareaValue"
          label="Project Notes"
          placeholder="Add any additional notes for the developer..."
          :rows="4"
          show-count
          :max-length="240"
        />
      ...
      <div class="bg-white border border-[#DEE1E7] p-5 rounded-xl space-y-4">
        <div class="flex items-center justify-between">
          ...
          <div
            class="flex items-center gap-1 bg-[#EEF0F4] p-1 rounded-lg text-xs font-semibold"
          >
            <button
              type="button"
              @click="sandboxOtpLength = 4"
              :class="[
        ...
        <LazyVOtp
          v-model="sandboxOtpValue"
          :length="sandboxOtpLength"
          :role-context="currentRole"
