<template>
  <div role="img" :aria-label="accessibleLabel" :class="[
    'relative inline-flex items-center justify-center rounded-full shrink-0 font-bold select-none',
    sizeStyles.box,
  ]">
    <!-- 1. Photographic Avatar Primitive -->
    <img v-if="src && !imageFailed" :src="src" :alt="alt || name"
      class="w-full h-full object-cover rounded-full border border-[#DEE1E7]" @error="imageFailed = true" />

    <!-- 2. Role-Accented Monogram / Initials Fallback Tile -->
    <div v-else :class="[
      'w-full h-full flex items-center justify-center rounded-full border tracking-wide font-bold',
      roleStyles.bg,
      roleStyles.text,
      roleStyles.border,
    ]">
    ...
    <span v-if="presence !== 'none' && !verified" :class="[
  ...
  switch (props.size) {
    case "xs":
      return {
        ...
        dot: "w-1.5 h-1.5 ring-[1.5px] -bottom-0.5 -end-0.5",
        badge: "w-3 h-3 text-[7px] ring-[1.5px] -bottom-1 -end-1",
      };
    case "sm":
      return {
        box: "w-8 h-8 text-[12px]",
        dot: "w-2 h-2 ring-[2px] bottom-0 end-0",
        badge: "w-3.5 h-3.5 text-[8px] ring-[2px] -bottom-0.5 -end-0.5",
      };
    case "lg":
      return {
        box: "w-14 h-14 text-[16px]",
        dot: "w-3 h-3 ring-[2.5px] bottom-0.5 end-0.5",
        badge: "w-5 h-5 text-[11px] ring-[2.5px] bottom-0 end-0",
      };
    case "xl":
      return {
        box: "w-[72px] h-[72px] text-[22px]",
        dot: "w-4 h-4 ring-[3px] bottom-1 end-1",
        badge: "w-6 h-6 text-[13px] ring-[3px] bottom-0.5 end-0.5",
      };
    case "md":
    default:
      return {
        box: "w-10 h-10 text-[14px]",
        dot: "w-2.5 h-2.5 ring-[2px] bottom-0 end-0",
        badge: "w-4 h-4 text-[9px] ring-[2px] -bottom-0.5 -end-0.5",
      };
  }
});
